...
export const getSearchSuggestions = (query, exclude) => {
  const queryObj = { query, exclude }

  return dispatch => {
    api.post(`/v1/search_suggestions`, queryObj)
      .then(results => {
        dispatch({ type: 'SET_SEARCH_SUGGESTIONS', data: results.data })
      })
  }
}
